 ...
 * @date 21-Nov-18
 */
abstract class BaseFeatureFragment : BaseFragment(), ComponentOwner<FeatureComponent> {

    override val component: FeatureComponent by lazy { buildComponent() }

    @Suppress("UNCHECKED_CAST")
    protected val appComponent: AppComponent
        get() = (activity?.application as? AppComponentOwner)?.component
    ...
    protected fun <T : Any> getProvidedComponent(classOfT: Class<T>): T {
        return (activity?.application as? ComponentProvider)?.getComponent(classOfT)
